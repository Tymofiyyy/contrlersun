// lib/services/api_service.dart - ПОВНА ВЕРСІЯ З ENERGY MODE
...
class ApiService {
  ...
  // Callback для оновлення токена (буде встановлений з AuthProvider)
  ...
  ApiService._internal() {
    ...
    // Автоматично додаємо токен до заголовків + обробка помилок
    _dio.interceptors.add(InterceptorsWrapper(
      ...
      onResponse: (response, handler) {
        // API працює - позначаємо як підключений
        _isConnected = true;
      ...
      onError: (error, handler) async {
        ...
        // Перевіряємо стан підключення при помилках мережі
  ...
  // Встановити callback для оновлення токена
  ...
  // Перевірка підключення з автоматичним відновленням
  Future<bool> checkConnection() async {
    // Якщо недавно перевіряли і все було ОК - не перевіряємо знову
    if (_isConnected &&
  ...
  // Безпечний виклик API з автоматичною перевіркою
  Future<T?> _safeApiCall<T>(Future<T> Function() apiCall,
      [String? operation]) async {
    try {
      // Якщо токена немає для захищених операцій
  ...
  // Отримання енергетичних даних з retry логікою
  ...
  // Додавання енергетичних даних з retry
  ...
  Future<bool> saveDailySummary(
    ...
    final result = await _safeApiCall(() async {
      ...
      await _dio.post(
        ...
        data: {
          'date': summary.date.toIso8601String().split('T')[0], // YYYY-MM-DD
  ...
  // Отримання денної статистики за діапазон дат
  ...
  // Отримання загальної статистики за всі дні
  ...
  // Отримання поточного режиму енергії
  ...
  // Швидке вимкнення/увімкнення розкладу
